...
@test "addon.apexos_role returns the role field" { # codespell:ignore apexos
    run get_field '{"slug":"alpha","apexos_role":"manager"}' bashio::app.apexos_role
    [ "${output}" = "manager" ]
}

@test "addon.apexos returns the minimal core version" {
    run get_field '{"slug":"alpha","apexos":"2024.1.0"}' bashio::app.apexos
    [ "${output}" = "2024.1.0" ]
}
...
@test "addon.apexos_api defaults to false when absent" { # codespell:ignore apexos
    run get_field '{"slug":"alpha"}' bashio::app.apexos_api
    [ "${output}" = "false" ]
}

@test "addon.apexos_core_api returns the value" {
    run get_field '{"slug":"alpha","apexos_core_api":true}' bashio::app.apexos_core_api
    [ "${output}" = "true" ]
}
