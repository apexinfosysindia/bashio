...
@test "host.operating_system returns the operating system" {
    bashio::api.supervisor() {
        printf '%s' '{"operating_system":"ApexOS OS 12.0"}'
    }
    run bashio::host.operating_system
    [ "${status}" -eq 0 ]
    [ "${output}" = "ApexOS OS 12.0" ]
}
...
@test "host.cpe returns the cpe" {
    bashio::api.supervisor() {
        printf '%s' '{"cpe":"cpe:2.3:o:apexos:apexos:12.0"}'
    }
    run bashio::host.cpe
    [ "${status}" -eq 0 ]
    [ "${output}" = "cpe:2.3:o:apexos:apexos:12.0" ]
}
