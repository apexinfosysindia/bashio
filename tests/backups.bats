...
@test "backup.apexos_version returns the version when present" {
    bashio::api.supervisor() { printf '%s' '{"apexos":"2024.1.0"}'; }
    run bashio::backup.apexos_version "aaa"
    [ "${status}" -eq 0 ]
    [ "${output}" = "2024.1.0" ]
}

@test "backup.apexos_version returns empty when missing" {
    bashio::api.supervisor() { printf '%s' '{}'; }
    run bashio::backup.apexos_version "aaa"
    [ "${status}" -eq 0 ]
    [ "${output}" = "" ]
...
@test "backup.apexos_exclude_database returns the flag when present" {
    bashio::api.supervisor() {
        printf '%s' '{"apexos_exclude_database":true}'
    }
    run bashio::backup.apexos_exclude_database "aaa"
    [ "${status}" -eq 0 ]
    [ "${output}" = "true" ]
}

@test "backup.apexos_exclude_database defaults to false when missing" {
    bashio::api.supervisor() { printf '%s' '{}'; }
    run bashio::backup.apexos_exclude_database "aaa"
    [ "${status}" -eq 0 ]
    [ "${output}" = "false" ]
...
@test "backup.apexos selects the slug from the full backup list" {
    bashio::api.supervisor() {
        echo "$2" >"${BATS_TEST_TMPDIR}/resource"
        printf '%s' '{"backups":[{"slug":"aaa","content":{"apexos":true}}]}'
    }
    run bashio::backup.apexos "aaa"
    [ "${status}" -eq 0 ]
    [ "${output}" = "true" ]
