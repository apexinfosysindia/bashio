...
@test "info.apexos extracts the ApexOS version" {
    bashio::api.supervisor() { printf '%s' "${INFO_JSON}"; }
    run bashio::info.apexos
    [ "${status}" -eq 0 ]
    [ "${output}" = "2024.6.1" ]
}

@test "info.apexos propagates an API failure" {
    bashio::api.supervisor() { return 1; }
    rc=0
    bashio::info.apexos || rc=$?
    [ "${rc}" -ne 0 ]
}
...
@test "info.os extracts the os version" {
    bashio::api.supervisor() { printf '%s' "${INFO_JSON}"; }
    run bashio::info.os
    [ "${status}" -eq 0 ]
    [ "${output}" = "12.3" ]
...
@test "info.os propagates an API failure" {
    bashio::api.supervisor() { return 1; }
    rc=0
    bashio::info.os || rc=$?
    [ "${rc}" -ne 0 ]
