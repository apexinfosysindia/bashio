...
# ------------------------------------------------------------------------------
# Returns the version of ApexOS that was in use when the backup was
# created.
#
# Arguments: $1 Backup slug
# ------------------------------------------------------------------------------
function bashio::backup.apexos_version() {
    local slug=${1}
    bashio::log.trace "${FUNCNAME[0]}" "$@"
    bashio::backups "${slug}" "backups.${slug}.apexos_version" '.apexos // empty'
}
...
# ------------------------------------------------------------------------------
# Returns if the ApexOS database file was excluded from this backup.
#
# Arguments:
#   $1 Backup slug
# ------------------------------------------------------------------------------
function bashio::backup.apexos_exclude_database() {
    local slug=${1}
    bashio::log.trace "${FUNCNAME[0]}" "$@"
    bashio::backups \
        "${slug}" \
        "backups.${slug}.apexos_exclude_database" \
        '.apexos_exclude_database // false'
}
...
# ------------------------------------------------------------------------------
# Returns if the backup contains apexos.
#
# Arguments:
#   $1 Backup slug
# ------------------------------------------------------------------------------
function bashio::backup.apexos() {
    local slug=${1}
    bashio::log.trace "${FUNCNAME[0]}" "$@"
    bashio::backups \
        false \
        "backups.${slug}.apexos" \
        ".backups[] | select(.slug == \"${slug}\") | .content.apexos"
}
